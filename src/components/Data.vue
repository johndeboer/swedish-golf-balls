...
<template>
  <div class="w-full flex flex-col items-center justify-center">
    ...
    <!-- Map Div -->
      <div class="md:w-[700px] lg:w-[800px] md:h-[600px] w-full h-[500px] px-2 md:p-0">
        <l-map
          ref="map"
          v-model:zoom="zoom"
          :center="center"
        ...
        >
          ...
          <l-marker
            ...
            v-for="store of ikea" @click="setLine(store.lat, store.lon, store.nearest_lat, store.nearest_lon)"
          ><l-icon
              ...
              :shadow-size="[41, 41]"
            ></l-icon></l-marker>
          <l-marker :lat-lng="[store.lat, store.lon]" v-for="store of topgolf" @click="setLine(store.lat, store.lon, store.nearest_lat, store.nearest_lon)">
            <l-icon
          ...
          <l-polyline :key="lineIteration" :lat-lngs="lineCoords.coords" color="red"></l-polyline>
        </l-map>
      </div> 
...
export default {
  ...
  methods:{
    setLine(aLat, aLon, bLat, bLon){
        ...
        this.$refs.map.leafletObject.fitBounds([[aLat, aLon], [bLat, bLon]])
  ...
  computed:{
    closest5(){
      return this.ikea.sort((a,b) => a.nearest_distance > b.nearest_distance).slice(0, 5);
    },
    farthest5(){
      return this.ikea.sort((a,b) => a.nearest_distance < b.nearest_distance).slice(0, 5);
    },
